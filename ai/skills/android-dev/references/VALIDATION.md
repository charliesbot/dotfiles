...
- Ambiguous platform request: ask whether the target is `:app`, `:wear`, both, `:widget`, `:complications`, or another surface before generating modules or writing platform UI.
...
- Widget or complication request: use root modules (`:widget`, `:complications`), not `:app`, `:wear`, or `:features:*`.
